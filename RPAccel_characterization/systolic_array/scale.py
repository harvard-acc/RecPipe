...
class scale:
    ...
    def cleanup(self):
        SS_BASE_DIR = './RPAccel_characterization/systolic_array'

        if not os.path.exists("{}/outputs/".format(SS_BASE_DIR)):
            os.system("mkdir {}/outputs".format(SS_BASE_DIR))

        net_name = self.topology_file.split('/')[-1].split('.')[0]

        path = "{}/outputs/scale_out".format(SS_BASE_DIR)
        if self.run_name == "":
            path = "{}/outputs/".format(SS_BASE_DIR) + net_name +"_"+ self.dataflow
        else:
            path = "{}/outputs/".format(SS_BASE_DIR) + self.run_name
