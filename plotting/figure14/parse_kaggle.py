...
def main():
    ...
    cpugpu_files  = ['logs/example_logs/figure7_8_kaggle1.out',
                     'logs/example_logs/figure7_8_kaggle2.out',
                     'logs/example_logs/figure7_8_kaggle3.out'
             ]

    accel_files  = ['logs/example_logs/figure12_kaggle1.out',
    ...
    accel_3stage  = list(filter(lambda x: x['stages'] == 3, accel_exps))
    ...
    experiments = [cpu_1stage, cpu_2stage, cpu_3stage,
                   gpu_1stage, gpu_2stage, gpu_3stage,
                   accel_1stage, accel_2stage, accel_3stage
                   ]
    labels = ["CPU 1 stage", "CPU 2 stage", "CPU 3 stage",
              "GPU 1 stage", "GPU 2 stage", "GPU 3 stage",
              "Accel 1 stage", "Accel 2 stage", "Accel 3 stage"]

    arrival_rates = [15.0, 2.5, 0.5]
    aids = ["Low", "Medium", "High"]
    ndcg_thres = 92.25

    for aid, arrival_rate in enumerate(arrival_rates):
        print("Results for ", aids[aid], "throughput")
        for i in range(len(experiments)):
                exp = list(filter(lambda x: x['arrival_rate'] <= arrival_rate, experiments[i]))
                exp = list(filter(lambda x: x['ndcg64'] * 100. >= ndcg_thres, exp))
                ...
                if exp[0]['tail_time'] * 1000. > 500:
                    ...
                    print(labels[i], exp[0]['tail_time'] * 1000. , "ms")
        print("--------")
